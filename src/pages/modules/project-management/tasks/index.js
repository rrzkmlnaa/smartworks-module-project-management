...
import TableHeader from 'src/views/modules/project-management/tasks/TableHeader'
...
import { fetchTask, updateTask } from 'src/libs/modules/project-management/task'
...
const StatusOptions = ({ id, status, reFetchTable }) => {
  const [anchorEl, setAnchorEl] = useState(null)
  ...
  return (
    <>
      ...
      >
        {textCapitalize(status)} <Icon icon='tabler:edit' />
      </Typography>
      <Menu
        keepMounted
        anchorEl={anchorEl}
        ...
        PaperProps={{ style: { minWidth: '8rem' } }}
      >
        {['new', 'in progress', 'completed', 'canceled'].map((_, idx) => (
          <MenuItem key={idx} onClick={() => handleStatusSelected(_)}>
            {textCapitalize(_)}
          </MenuItem>
        ))}
      </Menu>
...
const TasksPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      headerName: 'Status',
      renderCell: ({ row }) => {
        return <StatusOptions id={row.id} status={row.status} reFetchTable={getTask} />
      }
    },
    {
      flex: 0.1,
  ...
  const getTask = async () => {
    try {
      const res = await fetchTask({}, true)
      setProjects(res)
    } catch (error) {
